...
//		}
	...
	public static String[][] get_StringBinaryInstance (int n, int amount, float x1_min, float x1_max, float x2_min, float x2_max){
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < amount; j++) {
				if (j == 0)
					// WAT? генеруєм значення в діапазаоні; df.format обрізаєм лишні знаки(точність) отримуєм String(float); Float.parseFloat отримуєм Float(String); Float.floatToRawIntBits побітово переводимось в int(float); Integer.toBinaryString отримуєм двійковий код String->Int(Float)
